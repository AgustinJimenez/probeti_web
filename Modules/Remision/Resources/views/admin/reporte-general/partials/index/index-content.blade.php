<style type="text/css">
	...
	@foreach ($remisiones as $remision)
		<div obra-id="{{ $remision->obra->id }}" cliente-id="{{ $remision->obra->cliente->id }}"  fecha_remision = "{{ $remision->fecha_remision }}" estado = "{{ $remision->estado }}" terminado = "terminado-{{ $remision->terminado }}" class="remision" style="display:none;">
				...
				<div class="col-md-3">
					<table class="table table-bordered table-striped table-hover table-condensed">
						...
						<tr>
							...
							<td class="blue-bg">
								<strong>{{ $remision->obra->cliente->razon_social }}</strong>
							</td>
						</tr>
						<tr>
							<td class="blue-bg">
					...
					<table class="data-table table table-bordered table-hover table-condensed">
						...
						<tbody>
							...
							<tr class="chicas">
								...
								<td class="facturadas  probeta">{{ $facturadas_chicas = $datos_probetas['facturadas']['chicas'] }}</td>
								<td class="por_cobrar  probeta">{{ $por_cobrar_chicas = $datos_probetas['por_cobrar']['chicas'] }}</td>
							...
							<tr class="medianas">
								...
								<td class="facturadas probeta">{{ $facturadas_medianas = $datos_probetas['facturadas']['medianas'] }}</td>
								<td class="por_cobrar probeta">{{ $por_cobrar_medianas = $datos_probetas['por_cobrar']['medianas'] }}</td>
							...
							<tr class="grandes">
								...
								<td class="facturadas probeta">{{ $facturadas_grandes = $datos_probetas['facturadas']['grandes'] }}</td>
								<td class="por_cobrar probeta">{{ $por_cobrar_grandes = $datos_probetas['por_cobrar']['grandes'] }}</td>
							...
							<tfoot class="todas blue-bg">
								<tr>
									...
									<td class="facturadas todas-facturadas"><strong>{{ $facturadas_chicas + $facturadas_medianas + $facturadas_grandes }}</strong></td>
									<td class="por_cobrar todas-por_cobrar"><strong>{{ $por_cobrar_chicas + $por_cobrar_medianas + $por_cobrar_grandes }}</strong></td>
